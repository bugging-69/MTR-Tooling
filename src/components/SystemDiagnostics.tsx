...
`;
  ...
  const renderStatusBadge = (status: string) => {
    ...
    const isWarn = status.includes('WARN');
    const isFail = status.includes('FAIL');
    
    const cleanText = status.replace(/[✅⚠️❌]/g, '').trim() || (
      isPass ? 'Pass' :
      isWarn ? 'Warn' :
      isFail ? 'Fail' : 'Unknown'
    );
    ...
    if (isPass) {
      return (
        <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
    ...
    if (isWarn) {
      return (
        <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
    ...
    if (isFail) {
      return (
        <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-red-500/10 text-red-400 border border-red-500/20">
    ...
    return (
      <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-slate-500/10 text-slate-400 border border-slate-500/20">
