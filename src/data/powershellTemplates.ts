...
if ($dispCount -ge ${config.minimumDisplayCount} -and $dispCount -le 4) {
...
} elseif ($dispCount -gt 4) {
    $Results.DisplayCount = "WARN"
    $Details.DisplayCount = "$dispCount displays connected (MTR standard supports up to 4 displays)"
...
export function generateInstallTeamsRoomsScript(): string {
  return `<#
.SYNOPSIS
    Downloads and installs the newest Microsoft Teams Rooms App update.
#>
$ErrorActionPreference = "Stop"
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "   INSTALLING NEWEST MICROSOFT TEAMS ROOMS APP" -ForegroundColor Yellow
Write-Host "==========================================================" -ForegroundColor Cyan

$Url = "https://go.microsoft.com/fwlink/?linkid=2151817"
$DownloadPath = Join-Path $env:TEMP "MTR-Update.ps1"

Write-Host "Downloading MTR Update Script from $Url ..."
Invoke-WebRequest -Uri $Url -OutFile $DownloadPath

Write-Host "Unblocking downloaded script..."
Unblock-File -Path $DownloadPath

Write-Host "Executing MTR Update Script..."
& $DownloadPath
...
.SYNOPSIS
    ...
    Also ensures TPM is running and pushes all Windows updates.
#>
$ErrorActionPreference = "Continue"
...
Write-Host "[1/4] Checking and enabling TPM 2.0..."
$tpm = Get-Tpm -ErrorAction SilentlyContinue
if ($tpm) {
    if (-not $tpm.TpmReady) {
        Write-Host " TPM is present but not ready. Attempting to enable..." -ForegroundColor Yellow
        Enable-TpmAutoProvisioning
        Initialize-Tpm
        Write-Host " TPM enabled." -ForegroundColor Green
    } else {
        Write-Host " TPM is already running and ready." -ForegroundColor Green
    }
} else {
    Write-Host " Could not query TPM or TPM is not present." -ForegroundColor Red
}

Write-Host "[2/4] REQUESTED: Windows Update interactive scan..." -NoNewline
try {
...
Write-Host "[3/4] REQUESTED: Pushing all Windows Updates..."
try {
    Start-Process -FilePath "UsoClient.exe" -ArgumentList "StartInstall" -Wait -NoNewWindow -PassThru
    Write-Host " Updates push requested." -ForegroundColor Green
} catch {
    $OperationFailed = $true
    Write-Host " FAILED to push updates: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host "[4/4] REQUESTED: Re-register existing SkypeRoomSystem package (repair only)..." -NoNewline
try {
...
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "Requested scan and repair steps finished." -ForegroundColor Cyan
